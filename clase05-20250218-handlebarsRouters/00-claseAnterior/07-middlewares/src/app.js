...
const { log, log2, log3 } = require('./middlewares/log.js');
const { format } = require('./middlewares/format.js');
...
app.post("/api/nombres", log2, log3, format, auth, (req, res)=>{
    let {nombre}=req.body
    if(!nombre){
        res.setHeader('Content-Type','application/json');
...
app.put("/api/nombres/:pos", auth, (req, res)=>{
    let {pos} = req.params
    pos=Number(pos)
    if(isNaN(pos)){
        res.setHeader('Content-Type','application/json');
...
app.delete("/api/nombres/:pos", auth, (req, res)=>{
    let {pos} = req.params
    pos=Number(pos)
    if(isNaN(pos)){
        res.setHeader('Content-Type','application/json');
